#!/usr/bin/env python3
"""
HOME WIN PREDICTION SYSTEM - SOCCERBASE VERSION
================================================
Predicts home team wins based on home/away form analysis.
"""
...
def apply_home_win_algorithm(home_data_6, away_data_6):
    ...
    if len(home_data_6) < 6 or len(away_data_6) < 6:
        return None, None, {"error": "Insufficient data (need 6 matches minimum)"}, False

    # H1: Home team not to lose in 5 out of last 6 home matches
    home_not_lost = sum(1 for m in home_data_6 if m["result"] != "L")
    if home_not_lost >= 5:
        passed.append("H1")
        if home_not_lost == 6:
            details["H1"] = f"PERFECT PASS ({home_not_lost}/6 no losses)"
        else:
            details["H1"] = f"PASS ({home_not_lost}/6 no losses)"
            is_perfect = False
    else:
        failed.append("H1")
        details["H1"] = f"FAIL ({home_not_lost}/6, need 5+ no losses)"
        is_perfect = False

    # H2: Home team scored 10+ goals in last 6 home matches
    home_goals_scored = sum(m["gf"] for m in home_data_6)
    if home_goals_scored >= 10:
        passed.append("H2")
        details["H2"] = f"PASS ({home_goals_scored} goals scored)"
    else:
        failed.append("H2")
        details["H2"] = f"FAIL ({home_goals_scored}, need 10+)"
        is_perfect = False

    # H3: Home team conceded less than 5 goals in last 6 home matches
    home_goals_conceded = sum(m["ga"] for m in home_data_6)
    if home_goals_conceded < 5:
        passed.append("H3")
        details["H3"] = f"PASS ({home_goals_conceded} goals conceded)"
    else:
        failed.append("H3")
    ...
    if last_2_home_wins == 2:
        passed.append("H5")
        details["H5"] = f"PERFECT PASS (won last 2/2)"
    else:
        failed.append("H5")
        details["H5"] = f"FAIL ({last_2_home_wins}/2, need 2 wins)"
        is_perfect = False
    ...
    if away_losses >= 2:
        ...
        failed.append("A1")
        details["A1"] = f"FAIL ({away_losses}/6, need 2+ losses)"
        is_perfect = False

    # A2: Away team conceded 10+ goals in last 6 away matches
    away_goals_conceded = sum(m["ga"] for m in away_data_6)
    if away_goals_conceded >= 10:
        passed.append("A2")
        details["A2"] = f"PASS ({away_goals_conceded} goals conceded)"
    else:
        failed.append("A2")
        details["A2"] = f"FAIL ({away_goals_conceded}, need 10+)"
        is_perfect = False

    # A3: Away team scored 5 or fewer goals in last 6 away matches
    away_goals_scored = sum(m["gf"] for m in away_data_6)
    if away_goals_scored <= 5:
        passed.append("A3")
        details["A3"] = f"PASS ({away_goals_scored} goals scored)"
    else:
        failed.append("A3")
    ...
    # A5: Away team did not win 4+ of last 6 away matches
    if away_wins < 4:
        passed.append("A5")
        details["A5"] = f"PASS ({away_wins}/6 wins, max 3)"
    else:
        failed.append("A5")
        details["A5"] = f"FAIL ({away_wins}/6, need < 4 wins)"
        is_perfect = False
...
def main(date_str=None, only_scheduled=False):
    ...
    print(f"[+] Starting HOME WIN analysis for Date: {date_str}...")
    ...
    for match in all_matches:
        ...
        if len(home_form_6) < 6 or len(away_form_6) < 6:
            insufficient_data_count += 1
            continue

        passed, failed, details, is_perfect = apply_home_win_algorithm(home_form_6, away_form_6)
        if passed is None:
            continue
        ...
        result = {
            ...
            "failed": failed,
            "details": details,
            "home_form": home_form_6,
            "away_form": away_form_6,
            "score": len(passed),
    ...
    # ==========================================
    # BUILD CLEAN EMAIL OUTPUT (COMPLETED)
    # ==========================================
    email = []
    email.append("🏠 DAILY HOME WIN PREDICTIONS RECAP")
    ...
    if close_calls:
        for idx, c in enumerate(close_calls, 1):
            m = c["match"]
    ...
    email.append("\n💡 STRATEGY REMINDER:")
    email.append("  - For high confidence HOME WIN selections, consider Double Chance (1X) for risk mitigation.")
    email.append("  - Always verify current team news, injuries, and suspensions before placing bets.")
    ...
    filename = f"home_win_predictions_{date_str}.json"
    with open(filename, "w") as f:
        json.dump(output_data, f, indent=2, default=str)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Soccerbase Home Win prediction algorithm.")
