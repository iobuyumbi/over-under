...
def main():
    ...
    for day_offset in range(scan_days):
        ...
        if args.scheduled:
            fixtures = [f for f in fixtures if str(f.get("status", "")).lower() == "scheduled"]
        for m in fixtures:
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(process_single_match, m, m["date"], args.odds): m
    ...
    apply_portfolio_kelly(
        perfect + qualified + close, None, args.bankroll, MAX_TOTAL_EXPOSURE,
    )
